...
def cal_v_vmax(sdss_mags, sdss_mag_errs, gal_zs):
    m_max_r, survey_z_max, survey_z_min = 17.77 + 0.01, 0.085, 0.02
    ...
    ab_mags_r = ab_mags[:,2]
    zmax_list = []
    for ab_mag_r, coeff, gal_z in tqdm(zip(ab_mags_r, coeffs, gal_zs)):
...
def cal_v_vmax_mp(sdss_mags, sdss_mag_errs, gal_zs):
    v_vmax_list = []
    cpu_num = mp.cpu_count()
    with mp.Pool(processes=cpu_num) as p:
        v_vmax_list.append(p.starmap(cal_v_vmax, 
                    zip(np.array_split((sdss_mags),cpu_num), np.array_split((sdss_mag_errs),cpu_num), np.array_split((gal_zs),cpu_num))))
